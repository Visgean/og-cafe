...
from Lisculea.Cafe.models import Product, Order, SubOrder
...
def new(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect("/cafe/order/today")
    else:
...
@csrf_protect
@permission_required("Cafe.add_order")
def display_order_formset(request, queryset, redirect = "/cafe/orders/unpaid"):    
    if request.method == 'POST':
...
def unpaid(request):
    ...
    return display_order_formset(request, Order.objects.filter(paid = False), redirect = "/cafe/order/unpaid")

def today(request):
    return display_order_formset(request, Order.objects.filter(day__startswith = datetime.datetime.today().date()), redirect = "/cafe/order/today")
